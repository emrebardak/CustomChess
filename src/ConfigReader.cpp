...
void ConfigReader::parsePieces(const nlohmann::json &json) {
  ...
  for (const auto &pieceJson : json["pieces"]) {
    ...
    // Parse symbols
    if (pieceJson.contains("symbol")) {
      const auto &symbols = pieceJson["symbol"];
      if (symbols.contains("white")) {
        piece.symbol["white"] = symbols["white"].get<std::string>();
      }
      if (symbols.contains("black")) {
        piece.symbol["black"] = symbols["black"].get<std::string>();
      }
    }
...
void ConfigReader::parsePortals(const nlohmann::json &json) {
  ...
  for (const auto &portalJson : json["portals"]) {
    ...
    // Parse basic properties
    portal.type = portalJson.value("type", "");
    portal.id = portalJson.value("id", "");
    portal.symbol = portalJson.value("symbol", "⭕");  // Default portal symbol
    ...
    if (portalJson.contains("positions")) {
      ...
      if (positions.contains("entry")) {
        const auto &entry = positions["entry"];
        portal.positions.entry.x = entry.value("x", 0);
        portal.positions.entry.y = entry.value("y", 0);
      }

      // Parse exit position
      if (positions.contains("exit")) {
        const auto &exit = positions["exit"];
        portal.positions.exit.x = exit.value("x", 0);
        portal.positions.exit.y = exit.value("y", 0);
      }
    }

    // Parse properties
    if (portalJson.contains("properties")) {
      const auto &props = portalJson["properties"];
      portal.properties.preserve_direction = props.value("preserve_direction", true);
      portal.properties.cooldown = props.value("cooldown", 1);

      if (props.contains("allowed_colors") && props["allowed_colors"].is_array()) {
        for (const auto &color : props["allowed_colors"]) {
          portal.properties.allowed_colors.push_back(color.get<std::string>());
        }
      }
    }
